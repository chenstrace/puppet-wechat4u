...
export class PuppetWechat4u extends PUPPET.Puppet {
  ...
  override async messageSendText(
    ...
    log.verbose('PuppetWechat4u', 'messageSendText(%s, %s)', conversationId, text)
    ...
    this.cacheMessageRawPayload.set(res.MsgID, <WebMessageRawPayload>messagePayload)
    log.verbose('PuppetWechat4u', 'messageSendText MsgID(%s)', res.MsgID)
    return res.MsgID
  }
  ...
  override async messageSendFile(
    ...
    log.verbose('PuppetWechat4u', 'messageSendFile(%s, %s)', conversationId, file)
    ...
    this.cacheMessageRawPayload.set(res.MsgID, <WebMessageRawPayload>messagePayload)
    log.verbose('PuppetWechat4u', 'messageSendFile MsgID(%s)', res.MsgID)
    return res.MsgID
  }
  ...
  override async messageForward(
     ...
     * 如何直接转发消息
     */
    const res = await this.wechat4u.forwardMsg(rawPayload, conversationid)
    const messagePayload = {
      Content: '',
      FromUserName: this.currentUserId,
      MsgId: res.MsgID,
    ...
    this.cacheMessageRawPayload.set(res.MsgID, <WebMessageRawPayload>messagePayload)
    log.verbose('PuppetWechat4u', 'messageForward MsgID(%s)', res.MsgID)
    return res.MsgID
  }
